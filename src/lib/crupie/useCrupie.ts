import {getCard, getCardDeck} from "../healpers/getCardDeck";
import {useMemo, useState} from "react";
import {tab} from "@testing-library/user-event/dist/tab";

const stepsName = {
    1: "Blinds",
    2: "Preflop",
    3: "Flop",
    4: "Turn",
    5: "River",
    6: "Open"
}

function crupieLoop({step, setStep, hand1, table, deck, setTable}: any) {
    console.log(table)
    if (step === 1) {
        console.log("Blinds")
        setStep(2)
    } else if (step === 2) {
        console.log("Preflop")
        console.log(hand1)
        setStep(3)
    } else if (step === 3) {
        console.log("Flop")
        appendTable({step, setStep, deck, setTable})
        console.log(table)
        setStep(4)
    } else if (step === 4) {
        console.log("Turn")
        appendTable({step, setStep, deck, setTable})
        console.log(table)
        setStep(5)
    } else if (step === 5) {
        console.log("River")
        appendTable({step, setStep, deck, setTable})
        console.log(table)
        setStep(6)
    } else if (step === 6) {
        console.log("Open")
        console.log(table)
    }

}

function appendTable({table, setDeck, deck, setTable}: any) {
    if (table.length === 0) {
        for (let i = 0; i < 3; i++) {
            setTable([...table, getCard(deck, setDeck)])
        }
    } else if (table.length === 3 || table.length === 4) {
        setTable([...table, getCard(deck, setDeck)])
    }
}

export function useCrupie(deck: any, setDeck: any, table: any, setTable: any, step: any, setStep: any) {
    const card2 = useMemo(() => getCard(deck, setDeck), [])
    const card1 = useMemo(() => getCard(deck, setDeck), [])

    const [hand1, setHand1] = useState([card1, card2])

    return () => crupieLoop({step, setStep, hand1, table, deck, setTable})
}


/*
function getFlop()
{

}
function getTurn(){

}
function getRiver()
{

}*/
